...
def get_gemini_client():
    key = st.session_state.get("ui_gemini_key", "").strip()
    if not key:
...
def draw_boxes(img, detected):
    vis = img.copy()
    for e in detected:
        cv2.rectangle(vis, (e["box"][0], e["box"][1]), (e["box"][2], e["box"][3]), (0,255,0), 2)
    return vis
...
def _easyocr_single_crop(crop, reader):
    if _is_blank_crop(crop): return ""
    enhanced = camscanner_enhance(crop)
    gray = cv2.cvtColor(enhanced, cv2.COLOR_BGR2GRAY)
    try:
        results = reader.readtext(gray, detail=0, paragraph=True, contrast_ths=0.05, adjust_contrast=0.7)
...
def ocr_crops(crops):
    ...
    with ThreadPoolExecutor(max_workers=2) as ex:
        futures = {}
        for i, crop in enumerate(crops):
            futures[ex.submit(_ocr_single_crop, (i, crop, client, easyocr_texts[i]))] = i
            time.sleep(0.1)
        done = 0
...
def validate_gemini_key(key):
    try:
        import google.generativeai as genai_legacy
        genai_legacy.configure(api_key=key)
        model = genai_legacy.GenerativeModel(model_name="gemini-2.5-flash")
...
def parse_llm_json(raw):
    c = raw.strip()
    if c.startswith("```json"): c = c[7:].strip()
    elif c.startswith("```"): c = c[3:].strip()
...
""", unsafe_allow_html=True)
    ...
    with st.expander("🔑 API Keys", expanded=False):
        st.markdown('<span class="sec-label">Bring your own keys</span>', unsafe_allow_html=True)
        st.text_input(
            "Gemini API Key",
            type="password",
            placeholder="Required — aistudio.google.com",
            key="ui_gemini_key",
        ...
        if st.button("🔍 Validate Keys", use_container_width=True):
            gemini_key = st.session_state.get("ui_gemini_key", "").strip()
            groq_key   = st.session_state.get("ui_groq_key", "").strip()
            with st.spinner("Validating…"):
                results = {
                    "gemini": validate_gemini_key(gemini_key) if gemini_key else (None, "No key entered"),
                    "groq":   validate_groq_key(groq_key)     if groq_key   else (None, "No key entered"),
                }
            st.session_state.key_validation = results

        if "key_validation" in st.session_state:
            for service, label in [("gemini", "Gemini"), ("groq", "Groq")]:
                ok, err = st.session_state.key_validation.get(service, (None, ""))
                if ok is True:
                    st.markdown(f'<div style="font-size:12px;padding:4px 0;">✅ <b>{label}</b> — valid</div>', unsafe_allow_html=True)
                elif ok is False:
                    msg = err.split("\n")[0][:80] if err else "Invalid key"
                    st.markdown(f'<div style="font-size:12px;padding:4px 0;color:#ef4444;">❌ <b>{label}</b> — {msg}</div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div style="font-size:12px;padding:4px 0;color:#94a3b8;">— <b>{label}</b> — {err}</div>', unsafe_allow_html=True)
    ...
    conf_thresh = st.slider("Confidence Threshold", 0.0, 1.0, 0.3, 0.05, key="conf_thresh")
...
pages_data = st.session_state.get("pages_data", [{"img": st.session_state.img, "predictions": st.session_state.predictions}])
...
if process_btn:
    if not selected_fields:
        ...
        if missing:
            st.error(f"Please enter your {' and '.join(missing)} API key(s) in the sidebar under 🔑 API Keys.")
            st.stop()
        all_crops = []
        ...
        st.toast(f"Cropped {len(all_crops)} lines across {num_pages} page(s) — starting two-step OCR…")
        ...
        if '"error":"missing_gemini_key"' in llm_response:
            st.error("Please enter your Gemini API key in the sidebar under 🔑 API Keys.")
            st.stop()
...
with tab_images:
    for page_idx, (pd, p_det) in enumerate(zip(pages_data, per_page_detected)):
        ...
        if crops_preview:
            st.markdown(f'<span class="sec-label" style="margin-top:12px;display:block">Crop Strip Preview — {len(crops_preview)} regions</span>', unsafe_allow_html=True)
    ...
    if "ocr_text" in st.session_state:
        ...
        with hc2:
            st.download_button("📥 .txt", data=ocr_text.encode("utf-8"),
                               file_name="ocr_output.txt", mime="text/plain", use_container_width=True)
